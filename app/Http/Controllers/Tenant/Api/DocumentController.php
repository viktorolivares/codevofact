...
class DocumentController extends Controller
{
    ...
    public function send(Request $request)
    {
        if ($request->has('external_id')) {
            ...
            if (!$document) {
                throw new \RuntimeException("El documento con código externo {$external_id}, no se encuentra registrado.");
            }
            if ($document->group_id !== '01') {
                throw new \RuntimeException("El tipo de documento {$document->document_type_id} es inválido, no es posible enviar.");
            }
            $fact = new Facturalo();
