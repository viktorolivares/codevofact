...
class Functions
{
    ...
    public static function validateUbigeo($ubigeo) {
        ...
        if (strlen($ubigeo) > 0 && strlen($ubigeo) != 6 ) throw new \RuntimeException("El código ubigeo debe contener 6 dígitos");

        if (strlen($ubigeo) == 6) {
            $query_distric = District::where('id', $ubigeo)->first();

            if (!$query_distric) throw new \RuntimeException("El código ubigeo es incorrecto");
        }
    }
    ...
    public static function findAffectedDocumentByExternalId($external_id) {
        ...
        if (!$document) throw new \RuntimeException("No se encontró el documento con código externo {$external_id}.");
    ...
    public static function voidedDocuments($inputs, $type) {
        if (count($inputs['documents']) === 0) {
            throw new \RuntimeException("No se enviaron documentos para la anulación.");
        }
        ...
        foreach ($inputs['documents'] as $row) {
            ...
            if (!$document) throw new \RuntimeException("El código externo {$row['external_id']} no fue encontrado o la fecha indica no corresponde al documento.");
    ...
    public static function DNI($inputs){
        if (($inputs['document_type_id'] == '03') && ($inputs['total']) > 700) {
            ...
            if (!in_array($person->identity_document_type_id, ['01','04','06','07'])) throw new \RuntimeException("El tipo doc. identidad {$person->identity_document_type->description} del cliente no es valido.");
        }
    }
